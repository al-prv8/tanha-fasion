...
export default function AdminPage() {
  ...
  const handleBookSteadfast = async (orderId: string, codAmount: number, note: string) => {
    try {
      const res = await authenticatedFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/orders/${orderId}/book-steadfast`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ codAmount, note })
      });
      if (res.ok) {
        alert("অর্ডারটি সফলভাবে Steadfast কুরিয়ারে বুক করা হয়েছে!");
        fetchData();
        return await res.json();
      } else {
        const err = await res.json();
        throw new Error(err.error || "বুকিং করতে ব্যর্থ হয়েছে");
      }
    } catch (err: any) {
      console.error(err);
      alert(err.message);
  ...
  const handleSyncSteadfast = async (orderId: string) => {
    try {
      const res = await authenticatedFetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/orders/${orderId}/sync-steadfast`, {
        method: "POST"
      });
      if (res.ok) {
        const data = await res.json();
        alert(`কুরিয়ার ট্র্যাকিং সফলভাবে সিঙ্ক হয়েছে! বর্তমান স্ট্যাটাস: ${data.courierStatus}`);
        fetchData();
        return data;
      } else {
        const err = await res.json();
        throw new Error(err.error || "সিঙ্ক করতে ব্যর্থ হয়েছে");
      }
    } catch (err: any) {
      console.error(err);
      alert(err.message);
